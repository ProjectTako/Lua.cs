    ...
    partial class Lexer : IEnumerable<Token>
    {
        ...
        public IEnumerator<Token> GetEnumerator()
        {
            ...
            while( _index < _length )
            {
                ...
                {
                    throw new LuaCompilerException( _fileName, _currentLine, CompilerError.UnexpectedCharacter, ch );
                }
        ...
        private bool TryLexString( char ch, out Token token )
        {
            if( ch == '\"' || ch == '\'' || IsNext( "[[" ) )
            {
                var stringTerminator = !TakeIfNext( "[[" ) ? TakeChar().ToString() : "]]";
                var stringContentsBuilder = new StringBuilder();

                while( true )
                {
                    if( _index >= _length )
                        throw new LuaCompilerException( _fileName, _startLine, CompilerError.UnterminatedString );

                    if( TakeIfNext( stringTerminator ) )
                        break;

                    ch = TakeChar();

                    if( stringTerminator != "]]" && ( ch == '\n' || ch == '\r' ) )
                        throw new LuaCompilerException( _fileName, _startLine, "Unterminated string." );
                    ...
                    if( _index >= _length )
                        throw new LuaCompilerException( _fileName, _currentLine, CompilerError.UnexpectedEofString );
                    ...
                    switch( ch )
                    {
                        ...
                        case 'X':
                            ...
                            if( !ushort.TryParse( hex, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out hexValue ) )
                                throw new LuaCompilerException( _fileName, _currentLine, CompilerError.InvalidEscapeSequence, ch + hex );

                            stringContentsBuilder.Append( (char)hexValue );
                            break;

                        default:
                            throw new LuaCompilerException( _fileName, _currentLine, CompilerError.InvalidEscapeSequence, ch );
                    }
                }
        ...
        private bool TryLexNumber( char ch, out Token token )
        {
            if( char.IsDigit( ch ) )
            {
                ...
                if( !TryParseNumber( numberContents, format, out number ) )
                    throw new LuaCompilerException( _fileName, _currentLine, CompilerError.InvalidNumber, format.ToString().ToLower(), numberContents );
        ...
        public char TakeChar()
        {
            ...
            if( result == '\r' && PeekChar( 1 ) != '\n' )
                throw new LuaCompilerException( _fileName, _currentLine, "'\\r' must be followed by '\\n'." );
        ...
        private bool TryParseNumber( string value, NumberFormat format, out double result )
        {
            ...
            switch( format )
            {
                ...
                default:
                    throw new LuaCompilerException( _fileName, _currentLine, "Unsupported NumberFormat" );
            }
        ...
        private static bool TryParseBase( string value, int fromBase, out uint result )
        {
            try
            {
                result = Convert.ToUInt32( value, fromBase );
                return true;
            }
            catch
            {
